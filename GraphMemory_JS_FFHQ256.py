...
def LoadModel2():
    ...
    threshold = 20
    gen = TSFramework.Give_GenerationFromTeacher(16)
    generatedImages = Transfer_To_Numpy(gen)
    name_generation = dataNmae + "_" + modelName + str(threshold) + ".png"
    Save_Image(name_generation, merge2(generatedImages[8:16], [2, 4]))
...
def main():
    ...
    TSFramework.batch_size = 8
    TSFramework.maxTrainingStep = 200
    ...
    for step in range(TSFramework.currentTraningTime,totalTrainingTime):
        ...
        memoryBuffer_Files = TSFramework.GiveMemorizedSamples_Files()
        #memoryBuffer = torch.tensor(memoryBuffer).cuda().to(device=device, dtype=torch.float)

        #batch = LoadCACDFromPath(batch)

        memoryBufferFiles_ = np.concatenate((memoryBuffer_Files,batch),0)

        TSFramework.currentComponent.Train_Cpu_WithFiles(epoch,memoryBufferFiles_)
    ...
    generatedImages = Transfer_To_Numpy(gen)
    name_generation = dataNmae + "_" + modelName + str(threshold) + ".png"
    Save_Image(name_generation,merge2(generatedImages[0:8], [2, 4]))
