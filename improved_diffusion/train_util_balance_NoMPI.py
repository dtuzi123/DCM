...
class TrainLoop_Balance_NoMPI:
    ...
    def train_Memory_System(self,epoch,dataX):
        batchSize = self.batch_size
    ...
    def train_self_Cpu(self,epoch,dataX,generatedX):
        batchSize = self.batch_size
        smallBatchSize = int(batchSize/2.0)
        count = int(np.shape(dataX)[0] / smallBatchSize)
        myCount = int(np.shape(generatedX)[0] / smallBatchSize)
        ...
        for i in range(epoch):
            ...
            for j in range(count):
                ...
                generatedBatch = generatedX[myj*smallBatchSize:(myj+1)*smallBatchSize]
                #newbatch = mytorch.cat([batchX,generatedBatch],dim=0)
                newbatch = np.concatenate((batchX,generatedBatch),0)
                newbatch = th.tensor(newbatch).cuda().to(device=device, dtype=th.float)
    ...
    def train_self_Single_Cpu(self,epoch,dataX):
        batchSize = self.batch_size
        smallBatchSize = batchSize
        count = int(np.shape(dataX)[0] / smallBatchSize)
        device = th.device("cuda:0" if th.cuda.is_available() else "cpu")
        ...
        for i in range(epoch):
            ...
            for j in range(count):
                batchX = dataX[j*smallBatchSize:(j+1)*smallBatchSize]
                #newbatch = mytorch.cat([batchX,generatedBatch],dim=0)
                #newbatch = np.concatenate((batchX,generatedBatch),dim=0)
                newbatch = batchX
                newbatch = th.tensor(newbatch).cuda().to(device=device, dtype=th.float)
