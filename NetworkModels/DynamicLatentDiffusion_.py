...
class LatentDiffusionComponent(nn.Module):
    def __init__(self,model,diffusion,args,inputsize,batchsize):
        super(LatentDiffusionComponent, self).__init__()
        self.model = model
        ...
        self.autoencoder = Autoencoder196(device,inputsize)
    ...
    def Transfer_To_Numpy(self,sample):
        mySamples = sample.unsqueeze(0).cpu().detach().numpy()#cuda().cpu()
    ...
    def Train_Small(self,epoch,memory):
        ...
        latentMemory = self.autoencoder.Give_LatentCode(memory)
...
class DynamicLatentDiffusion(MemoryUnitFramework):
    ...
    def CreateLatentDiffusion(self):
        ...
        model.to(dist_util.dev())
        newTeacher = LatentDiffusionComponent(model, diffusion,args,self.input_size,self.batch_size)
