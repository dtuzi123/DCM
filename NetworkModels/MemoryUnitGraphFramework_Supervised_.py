...
class MemoryClusterSupervised:
    def __init__(self,device,x,y,maxSize,inputSize,TSF1,distance_type):
        self.device = device
    ...
    def AddSingleSample(self,x1,y1):
...
class MemoryUnitGraphFrameworkSupervised(DynamicDiffusionMixture):
    ...
    def MemoryBegin(self,x1,y1):
        new1 = MemoryClusterSupervised(self.device, x1,y1,128,self.input_size,self,self.distance_type)
            ...
            if np.shape(self.MemoryClusterArr)[0] < self.MaxMemoryCluster+1:
                ...
                if minDistance > self.threshold:
                    ...
                    if isBuild == False:
                        ...
                        newMemory = MemoryClusterSupervised(self.device,arr,arry,128,self.input_size,self,self.distance_type)
                        self.MemoryClusterArr.append(newMemory)
                        self.memoryUnits = self.memoryUnits + 1
                else:
                    self.MemoryClusterArr[minIndex].AddSingleSample(data1,dataY1)
            else:
    ...
    def GiveMemorizedSamples(self):
        ...
        for i in range(np.shape(self.MemoryClusterArr)[0]):
            ...
            oldY = self.MemoryClusterArr[i].arrLabel
