...
from improved_diffusion.train_util_balance_NoMPI import TrainLoop_Balance_NoMPI
from improved_diffusion.train_util_balance_NoMPI_MultiGPU import TrainLoop_Balance_NoMPI_MultiGPU
import torch.nn as nn

class Teacher_NoMPI(nn.Module):
    def __init__(self,inputSize):
        super(Teacher_NoMPI, self).__init__()
    ...
    def Train_Self_ByDataLoad_Single(self,epoch,dataX):
        ...
        trainer = TrainLoop_Balance_NoMPI(
            model=model,
            diffusion=diffusion,
            data=data,
            batch_size=args.batch_size,
    ...
    def Train_Self(self,epoch,mydata):
        ...
        if self.isTrainer == 0:
            self.isTrainer = 1
            self.trainer = TrainLoop_Balance_NoMPI(
                model=model,
                diffusion=diffusion,
                data=data,
                batch_size=args.batch_size,
            ...
            self.trainer.train_self(epoch, mydata,mydata)
        else:
            self.trainer.train_self(epoch, mydata,mydata)

    def Train_Self_Cpu(self,epoch,mydata):
        args = self.args
        logger.log("creating data loader...")
        print("test")
        print(self.input_size)
        ...
        if self.isTrainer == 0:
            ...
            self.trainer.train_self_Cpu(epoch, mydata,mydata)
        else:
            self.trainer.train_self_Cpu(epoch, mydata,mydata)


class Balance_Teacher_NoMPI(Teacher_NoMPI):
    ...
    def Train_Self_(self,epoch,mydata,generatedData):
        ...
        if self.isTrainer == 0:
            ...
            self.trainer.train_self(epoch, mydata,generatedData)
        else:
            self.trainer.train_self(epoch, mydata,generatedData)

    def Train_Self_Cpu(self,epoch,mydata,generatedData):
        args = self.args
        logger.log("creating data loader...")
        ...
        if self.isTrainer == 0:
            ...
            self.trainer.train_self_Cpu(epoch, mydata,generatedData)
        else:
            self.trainer.train_self_Cpu(epoch, mydata,generatedData)


    def train_self_Single_Cpu(self,epoch,mydata):
        args = self.args
        logger.log("creating data loader...")
        ...
        if self.isTrainer == 0:
            ...
            self.trainer.train_self_Single_Cpu(epoch, mydata)
        else:
            self.trainer.train_self_Single_Cpu(epoch, mydata)


class SingleDiffusion_NoMPI(Teacher_NoMPI):
    def Train_Self_Memory(self,epoch,mydata):
        args = self.args
        logger.log("creating data loader...")
        ...
        if self.isTrainer == 0:
            ...
            self.trainer.train_Memory(epoch, mydata)
        else:
            self.trainer.train_Memory(epoch, mydata)

    def Train_Self_MemorySystem(self,epoch,mydata):
        args = self.args
        logger.log("creating data loader...")
        ...
        if self.isTrainer == 0:
            ...
            self.trainer.train_Memory_System(epoch, mydata,self.device)
        else:
            self.trainer.train_Memory_System(epoch, mydata,self.device)
