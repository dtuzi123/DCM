...
from improved_diffusion.train_util_balance_NoMPI_MultiGPU_Real import *
...
class DiffusionComponentMultiGPU(nn.Module):
    def __init__(self,model,diffusion,args,inputsize,batchsize):
        super(DiffusionComponentMultiGPU, self).__init__()
        self.model = model
    ...
    def Train_Small(self,epoch,memory):
        ...
        if self.isTrainer == 0:
            self.isTrainer = 1
            self.trainer = TrainLoop_Balance_NoMPI_MultiGPUReal(
...
class DynamicDiffusionMixtureMultiGPU(nn.Module):
    def __init__(self,name,device,input_size):
        super(DynamicDiffusionMixtureMultiGPU, self).__init__()
    ...
    def Create_NewComponent(self):
        ...
        newTeacher = DiffusionComponentMultiGPU(model, diffusion,args,self.input_size,self.batch_size)
